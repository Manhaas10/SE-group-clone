  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setUser(response.data); // Store the user object (with user.id & user.username)
      } catch (error) {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        // If user data is available, match and update the requests
        if (user) {
    ...
    if (user) fetchRequests(); // Ensure user data is available before fetching requests
  }, [user]); // Re-run when user data changes

  // Update Request Status
  const handleStatusUpdate = async (id, newStatus) => {
    try {
      // newStatus==="declined"? newStatus="rejected":newStatus="approved";
